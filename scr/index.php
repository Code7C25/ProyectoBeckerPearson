...
if (isset($_GET['lang'])) {
    ...
    $url = strtok($_SERVER["REQUEST_URI"], '?'); // limpia GET para evitar conflictos
    header("Location: $url");
    exit;
}
...
body { margin:0; background:var(--bg-color); color:var(--text-color); font-family:'Poppins',sans-serif; }
header { background:var(--header-bg); color:var(--header-text); padding:12px 40px; display:flex; align-items:center; justify-content:center; gap:15px; position:relative; }
header img.logo { width:45px; }
.usuario { position:absolute; right:40px; top:15px; }
.usuario a { color:var(--header-text); text-decoration:none; margin-left:12px; }
.container { max-width:800px; margin:40px auto; padding:0 18px; }
.post { background:var(--post-bg); border-left:6px solid var(--header-bg); padding:20px; margin-bottom:30px; border-radius:10px; box-shadow:0 4px 16px rgba(0,0,0,0.07); }
.post h3 { color:var(--post-title); }
.post p.meta { color:var(--post-meta); font-size:13px; }
.post img.main-img { width:100%; max-height:300px; object-fit:cover; margin:10px 0; border-radius:8px; }
.icons a { font-size:22px; margin-right:10px; text-decoration:none; }
.comentarios { margin-top:20px; padding:15px; background:var(--post-bg); border-radius:10px; border:1px solid #ddd; }
.comentario { padding:8px 0; border-bottom:1px solid #bbb; }
.comentario:last-child { border-bottom:none; }
.fecha { font-size:12px; color:var(--post-meta); }
.btn-admin { margin-top:10px; }
.btn-edit, .btn-delete { padding:8px 12px; border-radius:6px; text-decoration:none; color:white; font-size:14px; }
.btn-edit { background:#1e88e5; }
.btn-delete { background:#d32f2f; }
.toggle-dark { position:fixed; right:20px; bottom:20px; font-size:22px; padding:10px; border-radius:50%; cursor:pointer; }
.toggle-lang { position:fixed; left:20px; bottom:20px; font-size:16px; padding:8px 12px; border-radius:8px; cursor:pointer; }
</style>
...
<form method="GET" class="toggle-lang">
    <button type="submit" name="lang" value="<?= $lang === 'es' ? 'en' : 'es' ?>"><?= $lang === 'es' ? 'EN' : 'ES' ?></button>
</form>
    ...
    <div class="comentarios">
        ...
        <?php while ($r = mysqli_fetch_assoc($reac)): ?>
            <div class="comentario">
                <strong><?= $r['NICKNAME'] ?></strong> <?= t('reaccionó con','reacted with') ?>
                <?= $r['TIPO'] == "corazon" ? "❤️" : "🙂" ?>
